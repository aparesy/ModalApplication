...
int main(int argc, char* argv[]){
	...
	albi = cmok;
	s2 = &albi;
	// Now albi and cmok are almost the same object except for the latency field and status that are not pointers
	// s2 is a pointer to albi
	modify_by_pointer(s2, "albi", 9000, "down");
	puts("--*s2--");
	print_idserver(*s2); // Just as expected
	puts("--albi--");
	print_idserver(albi); // Also just as expected
	puts("--cmok--");
	print_idserver(cmok); // As explained, almost everything is the same as albi except for latency and status
	puts("========================="); 
	puts("");
	...
	puts("=========================");
	puts("");
	free(s3); // I took the liberty to free it at the end
	// s used to be destroyed when we went outside the scope.
	// Now its fine but since its C there are some issues like if the function is called with a too large status for example.
